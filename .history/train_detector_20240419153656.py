...
def main(gpu, config, overrides):
    ...
    for epoch in range(start_epoch, config.optimizer.num_epochs):
        ...
        epoch_durations.append(epoch_duration)  # Storing epoch duration
        print(f"Epoch {epoch} duration: {epoch_duration:.2f} seconds.")

        for lr_scheduler in schedulers:
            lr_scheduler.step()

    # Save epoch durations at the end of training
    if rank == 0:
        with open('epoch_durations.json', 'w') as f:
            json.dump(epoch_durations, f)
